...
- For **arm64**: calls `build_arm64_deps()` which cross-builds the full dep stack (zlib, xz, openssl, libxml2, libogg, libvorbis, faad2, flac, lame, mp4v2, libshout, **Python 3.12.10**, **Perl 5.38.4**) from source into `/opt/ices0-static/arm64` using the Bootlin toolchain. Python is cross-compiled with `--with-build-python=/usr/local/python-static/bin/python3`; a host-runnable `python3-config` wrapper is installed to `${ARM64_PREFIX}/bin`. Perl is cross-compiled with `-Dhostperl=/usr/local/perl-static/bin/perl`; only `make libperl.a` is run (not the full `make` + `make install`) because the resulting arm64 `perl` binary cannot be executed on the amd64 build host — headers and `libperl.a` are copied manually to the CORE embed path; a `perl-embed-wrap` wrapper (returning arm64 embed flags without executing the arm64 binary) is symlinked as `perl`.
